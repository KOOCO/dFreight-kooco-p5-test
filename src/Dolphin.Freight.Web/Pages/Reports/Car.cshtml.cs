...
namespace Dolphin.Freight.Web.Pages.Reports
{
    ...
    {
        public CarViewModel InfoModel { get; set; }
        private readonly IGeneratePdf _generatePdf;
        public CarModel(IGeneratePdf generatePdf)
        {
            _generatePdf = generatePdf;
        }
        public void OnGet()
        {
            if (TempData["PrintDataCAR"] != null)
            {
                InfoModel = JsonConvert.DeserializeObject<CarViewModel>(TempData["PrintDataCAR"].ToString());
            }
            else
            {
                InfoModel = new  CarViewModel();
            }
